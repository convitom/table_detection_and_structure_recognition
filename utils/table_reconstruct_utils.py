...
class TableReconstructor:
    def __init__(self, 
                 use_ocr=True, 
                 ocr_lang="en",
                 # Text bbox merge thresholds for column span cells
                 col_span_merge_vertical_threshold=30,
                 col_span_merge_horizontal_threshold=50,
                 # Text bbox merge thresholds for row span cells
                 row_span_merge_vertical_threshold=5,
                 row_span_merge_horizontal_threshold=150):
        """
        ...
        Args:
            ...
            col_span_merge_vertical_threshold: Max vertical distance (y-axis) to merge text boxes in column span cells (default: 5)
            col_span_merge_horizontal_threshold: Max horizontal distance between box centers to merge in column span cells (default: 50)
            row_span_merge_vertical_threshold: Max vertical distance (y-axis) to merge text boxes in row span cells (default: 5)
            row_span_merge_horizontal_threshold: Max horizontal distance between box centers to merge in row span cells (default: 50)
        """
        self.use_ocr = use_ocr
        self.col_span_merge_vertical_threshold = col_span_merge_vertical_threshold
        self.col_span_merge_horizontal_threshold = col_span_merge_horizontal_threshold
        self.row_span_merge_vertical_threshold = row_span_merge_vertical_threshold
        self.row_span_merge_horizontal_threshold = row_span_merge_horizontal_threshold
    ...
    def _merge_vertical_boxes(self, boxes, vertical_threshold=5, horizontal_threshold=50):
        """
        Merge text bounding boxes that are close together vertically (same column)
        
        Args:
            boxes: List of text bounding boxes (each box is a list of 4 points)
            vertical_threshold: Max vertical gap between boxes to merge
            horizontal_threshold: Max horizontal distance between box centers to be considered same column
        
        Returns:
            List of merged bounding boxes [x_min, y_min, x_max, y_max]
        """
        if len(boxes) == 0:
            return []
        ...
        for i in range(1, len(rects)):
            ...
            # Check if same column (x centers are close)
            x_avg_prev = (prev[0] + prev[2]) / 2
            x_avg_curr = (curr[0] + curr[2]) / 2
            
            # Check vertical gap
            y_gap = curr[1] - prev[3]
            
            if abs(x_avg_prev - x_avg_curr) < horizontal_threshold and y_gap < vertical_threshold:
                # Same group (same cell)
                current_group.append(curr)
            else:
                # Merge old group
                if current_group:
                    x_min = min(r[0] for r in current_group)
                    y_min = min(r[1] for r in current_group)
                    x_max = max(r[2] for r in current_group)
                    y_max = max(r[3] for r in current_group)
    ...
    def _detect_text_in_blob(self, img, bbox, blob_type, row_lines):
        ...
        try:
            ...
            # Merge text bboxes based on blob type
            if blob_type == 'col_span':
                # For column span cells, merge vertically close boxes
                merged_bboxes = self._merge_vertical_boxes(
                    text_bboxes, 
                    vertical_threshold=self.col_span_merge_vertical_threshold,
                    horizontal_threshold=self.col_span_merge_horizontal_threshold
                )
            else:  # row_span
                # For row span cells, also merge vertically close boxes
                merged_bboxes = self._merge_vertical_boxes(
                    text_bboxes,
                    vertical_threshold=self.row_span_merge_vertical_threshold,
                    horizontal_threshold=self.row_span_merge_horizontal_threshold
                )
            
            # Convert merged rectangles back to text_bboxes format
            merged_text_bboxes = []
            for rect in merged_bboxes:
                x_min, y_min, x_max, y_max = rect
                # Convert to 4-point format
                pts = [[x_min, y_min], [x_max, y_min], [x_max, y_max], [x_min, y_max]]
                merged_text_bboxes.append(pts)
            
            return merged_text_bboxes
        except:
            return []
    ...
    def _find_best_separators_combination(self, text_bboxes, candidates, blob_range, axis='col'):
        ...
        for box in text_bboxes:
            # Handle both merged format [x_min, y_min, x_max, y_max] 
            # and original format [[x1,y1], [x2,y2], ...]
            if isinstance(box[0], list):
                ...
                # Already in rect format [x_min, y_min, x_max, y_max]
                if axis == 'col':
                    text_rects.append((box[0], box[2]))
                else:  # row
                    text_rects.append((box[1], box[3]))
